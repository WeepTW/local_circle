...
test('save, edit, cancel/confirm delete, identity and mobile navigation',async({page,request})=>{
 ...
 await page.goto('/preferences');await login(page);await expect(page.getByText('從第一個喜好開始')).toBeVisible()
 ...
 await login(page, '2');await expect(page.getByText('從第一個喜好開始')).toBeVisible()
 await login(page, '1');await expect(page.locator('tbody tr')).toHaveCount(1)
...
test('admin ownership and stored XSS render safely in the actual browser',async({page,request})=>{
 ...
 try {
  await page.goto('/preferences');await login(page);await login(page, '3');await page.getByRole('link',{name:'商品管理',exact:true}).click()
  ...
  await login(page, '4');await page.getByRole('link',{name:'商品管理',exact:true}).click();await expect(page.getByRole('button',{name:'編輯商品',exact:true})).toHaveCount(1)
